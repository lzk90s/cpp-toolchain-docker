...
typedef struct AlgoContext {
    const AVClass *class;
    int cid;    // channel id
    char *msg_pipe;   // event listen pipe
    char *roi_string;
...
static av_cold int init(AVFilterContext *ctx) {
    int ret = 0;
    AlgoContext *s = ctx->priv;
    const char *DLL_NAME = "libvideo_filter.so";

    av_log(NULL, AV_LOG_DEBUG, "init \n");
    av_log(NULL, AV_LOG_INFO, "cid = %d, msg_pipe_path = %s, roi_string = %s\n", s->cid, s->msg_pipe, s->roi_string);

    if (s->cid < 0) {
        av_log(NULL, AV_LOG_ERROR, "Invalid param\n");
    ...
    ret = pf_VFilter_Init(s->msg_pipe, s->roi_string);
    if (0 != ret) {
...
static const AVOption algo_options[] = {
    ...
    { "msg_pipe", "the message pipe for event", OFFSET(msg_pipe), AV_OPT_TYPE_STRING, {.str = NULL}, 0, 0, FLAGS },
    { "roi_string", "the ROI location", OFFSET(roi_string), AV_OPT_TYPE_STRING, {.str = NULL }, 0, 0, FLAGS },
    { NULL }
};
...
AVFilter ff_vf_algo = {
    .name           = "algo",
    .description    = NULL_IF_CONFIG_SMALL("video filter vith algorithm"),
